...
fn basic_serialize_shape() {
    ...
    let json = p.to_json_string();
    // ryu (fast-float) emits "1.5"/"-2.5"; fallback emits same via Display
...
fn zero_copy_borrow_no_allocation() {
    ...
    // `b.name` is a slice of `input` — verify same pointer range.
...
fn escaped_string_rejects_borrow() {
    ...
    assert!(matches!(result, Err(r_json::Error::EscapedString)), "got: {result:?}");
}

// ── 5. String with escape sequences (owned) ───────────────────────────────────
...
// ── 8. rename_all SCREAMING_SNAKE_CASE ───────────────────────────────────────
...
fn skip_omits_field() {
    ...
    assert_eq!(v2.internal, 0); // Default::default()
}
...
fn skip_serializing() {
    ...
    // Deserializing the round-trip gives Default for cached.
    let v2 = SkipSer::from_json_str(&json).unwrap();
    assert_eq!(v2.value, 5);
    assert_eq!(v2.cached, "");
}

// ── 13. skip_deserializing ────────────────────────────────────────────────────
...
fn skip_deserializing() {
    ...
    assert_eq!(v.value, 3);
    assert_eq!(v.computed, 0); // skip_deserializing → Default
}

// ── 14. skip_serializing_if ───────────────────────────────────────────────────
...
fn deny_unknown_fields_errors() {
    ...
    assert!(matches!(result, Err(r_json::Error::UnknownField)));
}
...
fn unit_enum_unknown_variant() {
    let json = r#""Unknown""#;
    assert!(matches!(Status::from_json_str(json), Err(r_json::Error::UnknownVariant)));
}

// ── 23. Enum with rename_all ──────────────────────────────────────────────────
...
// ── 24. Unknown fields skipped (default behaviour) ───────────────────────────
...
#[test]
fn trie_dispatch_roundtrip() {
    let v = TrieStruct { apple: 1, apricot: 2, banana: 3 };
    let json = v.to_json_string();
    let v2 = TrieStruct::from_json_str(&json).unwrap();
    assert_eq!(v, v2);
}
...
// ── 28. Large struct (field-hint cache effectiveness) ────────────────────────
...
// ── 29. Serializer: NaN / Infinity → null ────────────────────────────────────
...
// ── 31. SIMD / SWAR scanner unit tests ───────────────────────────────────────

#[test]
fn swar_find_quote() {
    let input = b"hello world\" rest";
    let pos = r_json::simd::find_quote_or_backslash(input, 0);
    assert_eq!(pos, 11);
}
...
// ── 35. Edge cases: max/min integer values ───────────────────────────────────
...
// ── 36. Edge cases: deeply nested struct (5 levels, flat chain) ──────────────
...
// ── 37. Edge cases: zero-length strings ──────────────────────────────────────
...
// ── 38. Edge cases: string with ASCII control characters ─────────────────────
...
// ── 39. Unicode roundtrip: BMP characters ────────────────────────────────────
...
// ── 40. Unicode roundtrip: emoji / surrogate-range code points ───────────────
...
// ── 41. Numeric edge cases: float special values → null ──────────────────────
...
// ── 43. Numeric edge cases: u64::MAX roundtrip ───────────────────────────────
...
// ── 44. Numeric edge cases: i64::MIN roundtrip ───────────────────────────────
...
// ── 45. Deeply nested + recursive-like: Vec of Node ──────────────────────────
...
// ── 46. PHF dispatch test: wide struct with 12 fields ────────────────────────
...
// ── 47. Integer key comparison: short field names ────────────────────────────
...
// ── 48. All single-char field names ──────────────────────────────────────────
...
// ── 49. Serialization uses reasonable capacity (no 10x over-allocation) ──────

#[test]
fn serialization_uses_size_hint() {
    let v = Point { x: 1.5, y: 2.5 };
    let bytes = v.to_json_bytes();
    // Capacity should be at most 4× length (i.e., at most one doubling happened).
    assert!(
...
// ── 50. deny_unknown_fields: nested unknown value is skipped (lenient struct) ─

#[test]
fn deny_unknown_nested_value_skipped() {
    // Without deny_unknown_fields, a nested object/array as an unknown value
    // should be skipped entirely without error.
...
// ── 45. SWAR scanner correctness at all byte offsets ────────────────────────
...
// ── 46. fused read_key_colon: pretty-printed (spaced) JSON ───────────────────
...
// ── 47. Capacity hint: Vec should not wildly over-allocate ───────────────────
...
// ── 48. Unknown nested object and array values are skipped cleanly ───────────
